(()=>{
  ...
  function translateText(root=document){
    ...
    try{
      ...
      nodes.forEach(node=>{
        const p=node.parentElement;if(!p||['SCRIPT','STYLE','TEXTAREA','OPTION'].includes(p.tagName))return;
      ...
      document.querySelectorAll('option').forEach(o=>{const t=o.textContent.trim();const r=map[t];if(r)o.textContent=r});
      const placeholders={
        'Search item, vendor, barcode, SKU, brand...':'Buscar artículo, proveedor, código de barras, SKU, marca...',
        'Item not in inventory':'Artículo que no está en inventario',
        'case, box, each...':'caja, paquete, unidad...',
        'Delivery instructions, substitutions, special requests…':'Instrucciones de entrega, sustituciones, solicitudes especiales…',
        'Full name':'Nombre completo','Phone number':'Número de teléfono'
      };
  ...
  function ensureSelector(){
    ...
    if(!box){
      ...
      box.querySelector('select').onchange=async e=>{
        ...
        lang=next;localStorage.setItem('masoras_language',lang);translateText(document);
      };
    }
  ...
  async function loadLanguage(){
    try{
      ...
      localStorage.setItem('masoras_language',lang);
      ensureSelector();translateText(document);
  ...
  function enhanceInvite(){
    ...
    const label=document.createElement('label');label.innerHTML='Language<select id="inviteLanguage"><option value="en">English</option><option value="es">Español</option></select>';
    role.after(label);
  ...
  function rerun(){ensureSelector();enhanceInvite();translateText(document)}
  function start(){patchInviteInvoke();loadLanguage();setTimeout(rerun,700);setTimeout(rerun,1600);document.addEventListener('click',()=>setTimeout(rerun,80),true);}
