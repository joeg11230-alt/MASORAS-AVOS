(()=>{
  ...
  function ensureInviteUI(){
    ...
    if(document.querySelector('#inviteUserModal'))return;
    ...
    modal.innerHTML=`<div class="box" style="max-width:780px">
      ...
      <form id="inviteUserForm" style="margin-top:14px">
        <div class="form">
          <label>Name<input id="inviteName" placeholder="Full name"></label>
          <label>Email<input id="inviteEmail" type="email" required placeholder="name@example.com"></label>
          <label>Phone<input id="invitePhone" type="tel" placeholder="Phone number"></label>
          <label>Access Level<select id="inviteRole"><option value="viewer">View Only</option><option value="editor">Editor</option></select></label>
        </div>
        ...
        <div class="row" style="margin-top:16px"><button id="inviteCancel" type="button">Cancel</button><button class="primary" type="submit"><i class="bi bi-envelope-fill"></i> Send Invite Link</button></div>
  ...
  function openInvite(){
    ...
    if(!m)return;
    m.querySelector('#inviteName').value='';
    m.querySelector('#inviteEmail').value='';
    m.querySelector('#invitePhone').value='';
    m.querySelector('#inviteRole').value='viewer';
    m.querySelector('#inviteActive').checked=true;
    m.querySelectorAll('[name="inviteSection"]').forEach(c=>c.checked=false);
    m.querySelector('#inviteMsg').textContent='';
  ...
  async function sendInvite(e){
    e.preventDefault();
    const m=document.querySelector('#inviteUserModal');
    const msg=m.querySelector('#inviteMsg');
    ...
    const body={
      display_name:m.querySelector('#inviteName').value.trim()||null,
      email:m.querySelector('#inviteEmail').value.trim().toLowerCase(),
      phone:m.querySelector('#invitePhone').value.trim()||null,
      role:m.querySelector('#inviteRole').value,
      active:m.querySelector('#inviteActive').checked,
      sections
    };
    msg.textContent='Sending invitation…';
    const {data,error}=await db.functions.invoke('invite-app-user',{body});
    if(error){msg.textContent=error.message||'Could not send invitation.';return;}
    if(data?.error){msg.textContent=data.error;return;}
    msg.innerHTML='<span class="match">Invite sent to '+body.email+'.</span>';
    setTimeout(()=>{m.classList.remove('show');window.location.reload()},900);
  }

  function enhanceExistingUserEditor(){
    const modal=document.querySelector('#permissionUserModal');
    if(!modal)return;
    const title=modal.querySelector('#permModalTitle');
    if(title&&title.textContent==='Add User') title.textContent='User Permissions';
  }

  const observer=new MutationObserver(()=>{ensureInviteUI();enhanceExistingUserEditor()});
  observer.observe(document.documentElement,{childList:true,subtree:true});
  setTimeout(ensureInviteUI,700);
  setTimeout(ensureInviteUI,1600);
})();