(()=>{
  ...
  function init(){
    ...
    setTimeout(arrangeHome,1600);
    const app=document.querySelector('#app');
    if(app){new MutationObserver(()=>{if(app.hidden){landed=false;return;}setTimeout(landOnProfile,50);}).observe(app,{attributes:true,attributeFilter:['hidden']});}
    setTimeout(landOnProfile,500);setTimeout(landOnProfile,1200);
  }
