(()=>{
  ...
  async function showOrgExt(){
    ...
    view.querySelectorAll('.phone-ext-display').forEach(x=>x.remove());
    if(!ext)return;
    const phoneLine=[...view.querySelectorAll('.contact-line')].find(x=>/^Phone:/i.test(x.textContent.trim()));
    if(phoneLine){
      const s=document.createElement('span');s.className='phone-ext-display';s.textContent=' Ext. '+ext;phoneLine.appendChild(s);
    }
  }
  ...
  const boot=()=>{
    ...
    const vm=document.querySelector('#vendorModal');if(vm)new MutationObserver(()=>{if(vm.classList.contains('show'))fillVendorExt()}).observe(vm,{attributes:true,attributeFilter:['class']});
    const om=document.querySelector('#orgProfileModal');if(om)new MutationObserver(()=>{if(om.classList.contains('show'))fillOrgExt()}).observe(om,{attributes:true,attributeFilter:['class']});
    const pv=document.querySelector('#orgProfileView');if(pv)new MutationObserver(()=>setTimeout(showOrgExt,50)).observe(pv,{childList:true,subtree:true});
  };
