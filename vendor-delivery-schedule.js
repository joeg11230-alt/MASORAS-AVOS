(()=>{
  ...
  function removeLegacyField(){
    const old=document.querySelector('#vdays');
    if(old)old.closest('label')?.remove();
  }
  ...
  function updateSummary(){
    const s=document.querySelector('#deliveryScheduleSummary');
    if(s)s.textContent=summaryText();
  }

  function setCollapsed(collapsed){
    const body=document.querySelector('#deliveryScheduleBody');
    const btn=document.querySelector('#deliveryScheduleToggle');
    if(!body||!btn)return;
  ...
  function ensureScheduleUI(){
    ...
    if(!form)return;
    let wrap=document.querySelector('#vendorDeliverySchedule');
    if(wrap)return;
    ...
    wrap.innerHTML=`
      <button id="deliveryScheduleHeader" type="button" style="width:100%;border:0;border-radius:0;background:#fbfdff;padding:12px;display:flex;justify-content:space-between;align-items:center;gap:12px;text-align:left">
        <span><span style="font-weight:800">Allowed Delivery Days & Hours</span><span id="deliveryScheduleSummary" class="muted" style="display:block;margin-top:3px">No delivery days selected</span></span>
        <span id="deliveryScheduleToggle" aria-expanded="false" style="font-size:20px;font-weight:800">▸</span>
      </button>
      <div id="deliveryScheduleBody" style="display:none;padding:0 12px 12px">
        <div class="muted" style="margin-bottom:8px">Select the days this vendor may deliver and the allowed time window.</div>
        <div id="deliveryScheduleRows" style="display:grid;gap:7px"></div>
      </div>`;
    ...
    wrap.querySelector('#deliveryScheduleHeader').addEventListener('click',()=>{
      const body=wrap.querySelector('#deliveryScheduleBody');
    ...
    rows.querySelectorAll('.delivery-day-enabled').forEach(c=>c.addEventListener('change',()=>{
      ...
      rows.querySelector(`.delivery-start[data-day="${day}"]`).disabled=!c.checked;
      rows.querySelector(`.delivery-end[data-day="${day}"]`).disabled=!c.checked;
      updateSummary();
    }));
    rows.querySelectorAll('.delivery-start,.delivery-end').forEach(i=>i.addEventListener('change',updateSummary));
  }

  async function fillSchedule(){
    ensureScheduleUI();removeLegacyField();
  ...
  document.addEventListener('click',e=>{
    if(e.target.closest('#addVendor'))setTimeout(()=>{ensureScheduleUI();removeLegacyField();clearSchedule()},100);
  ...
  setTimeout(()=>{ensureScheduleUI();removeLegacyField()},700);
  setTimeout(()=>{ensureScheduleUI();removeLegacyField()},1600);
})();